...
class Crm extends CI_Controller {
    ...
    {
        $data['crm']    = $this->crm;
        $data['selKar'] = $this->crm->selKaryawan($id);
        ...
        $data['chklink'] = $this->crm->checklink($id);
    ...
    public function ajax_list($cons='')
    {
        ...
        foreach ($list as $crm) {
            ...
            $row[] = $crm->id;
            $row[] = '<span style="display: none;">'.$crm->date_created.'</span>'.date('d-m-Y H:i:s', strtotime($crm->date_created)).'<br><b>'.strtoupper($crm->divisi).' - '.$crm->nickname.' - '.$crm->sales_cabang.'</b>';
