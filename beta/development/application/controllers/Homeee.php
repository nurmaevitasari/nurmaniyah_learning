...
class Home extends CI_Controller {
	...
	public function logout(){
		...
		setcookie('karyawan_id', "", time() -3600, "/");
		setcookie('position_id', "", time() -3600, "/");
		setcookie('cabang', "", time() -3600, "/");
	...
	{
		$user = $_SESSION['myuser']['karyawan_id'];
		$sql = "UPDATE tbl_notification SET status = 1 WHERE modul = 7 AND modul_id = $w_id AND user_id = $user AND record_type = $rec_type";
		$this->db->query($sql);
