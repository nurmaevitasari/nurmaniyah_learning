...
class M_delivery extends CI_Model {
  ...
  {
      $sql = "SELECT nickname FROM tbl_loginuser
              WHERE karyawan_id IN (SELECT contributor FROM tbl_do_contributor WHERE do_id = '$do_id' AND published = 0 GROUP BY contributor) ORDER BY nickname ASC";
      $res = $this->db->query($sql)->result_array();
