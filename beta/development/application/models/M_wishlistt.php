	...
	class M_wishlist extends CI_Model
	{
		...
		public function wishlist()
		{
			...
			if($user['position_id'] == '1' OR $user['position_id'] == '2') {
				...
				$sql .= "LEFT JOIN tbl_wish_contributor co ON co.wish_id = wi.id 
					WHERE wi.wish_to = '$kar' OR wi.user = '$kar' OR co.contributor = '$kar' ";
			}
		...
		{
			$sql  = "SELECT wi.id, wi.title, wi.descriptions, wi.status, wi.wish_to, wi.progress, wi.user, ulg.nickname as user_name, wlg.nickname as wish_name, wi.date_created, wi.date_closed, po.id as point_id, po.point
		...
		public function getFiles($id)
		{
			$sql = "SELECT up.*, lg.nickname FROM tbl_upload_wish up
					...
					WHERE up.w_id = '$id' GROUP BY up.id";
			$res = $this->db->query($sql)->result_array();
        ...
        public function update_tariff($where, $data) 
        {
