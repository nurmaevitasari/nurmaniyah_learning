<?php if(! defined('BASEPATH')) exit('No direct script access allowed');    
    ...
    class Cash_model extends CI_Model
    {
        ...
        { 
            ...
            if (in_array($position, array('1','2','77','55','56', '57', '58', '95','88','89', '90', '91', '93', '100')))
            { 
                if($tipe_cash == '2') { 

                    switch ($cbg) { 
                        case 'Bandung':
                            $this->notification($id, $id, '14', '75');                    
                            break;
                        case 'Medan':
                            $this->notification($id, $id, '14', '60');
                            break;
                        case 'Surabaya':
                            $this->notification($id, $id, '14', '62');
                            break;        
                        case 'Cikupa': 
                            $this->notification($id, $id, '14', '18');
                            break;
                        default:
                            $this->notification($id, $id, '14', '5');  
                            break;
                    }
                }
                ...
                $this->db->where('id', $id);
                $this->db->update('tbl_cash', array('status' => $ov, 'level_approval' => 'Dir'));

                $this->AddLogCash($id, $ov);
                ...
                if($divisi AND empty($level) AND !empty($cbg) AND !in_array($pos_user, array('55','56', '57', '58', '95'))) { //appr staff cabang dan ada divisi
                    $this->db->where('id', $id);
                    $this->db->update('tbl_cash', array('level_approval' => 'Dir')); // kacab appr, lvl langsung dir karena appr selesai di kacab saja. 

                    //$this->notification($id, $id, '13', $kadiv);
                }elseif($divisi AND empty($level) AND !empty($cbg)) { // appr kacab dan ada divisi
                    $this->db->where('id', $id);
                    $this->db->update('tbl_cash', array('level_approval' => 'Dir')); //kacab tidak perlu approval. akan diperbaiki.

                    //$this->notification($id, $id, '13', '2');
                }elseif($divisi AND $level == 'Kacab') { 
                    $this->db->where('id', $id);
                    $this->db->update('tbl_cash', array('level_approval' => 'Dir')); //appr staff cabang ke kadiv. harusnya tidak perlu karena sebelumnya sudah aporove kacab.

                    //$this->notification($id, $id, '13', '2');
                }elseif(empty($level) AND !empty($cbg)) {
                    $this->db->where('id', $id);
                    $this->db->update('tbl_cash', array('level_approval' => 'Dir')); // appr staff cabang tanpa divisi. lvl langsung dir karena appr selesai di kacab. 

                    //$this->notification($id, $id, '13', '2');
                }elseif(empty($level) AND empty($cbg) AND in_array($pos_user, array('65','66','67','68','71','103'))) { // appr sales jakarta.
                    $this->db->where('id', $id);
                    $this->db->update('tbl_cash', array('level_approval' => 'Dir'));
                ...
                //$this->db->where('id', $id);
                //$this->db->update('tbl_cash', array('status' => $sales, 'level_approval' => 'Kacab'));
        ...
        --Staff jkt kecuali sales, langsung approval direksi. */
            ...
            $sql = "SELECT position_id, cabang, amount, divisi FROM tbl_karyawan kr
            ...
            if(in_array($position_id, array('1', '2'))) { //direksi bikin cash
                ...
                switch ($cabang) { //staff cabang bikin cash
                    ...
                    default:
                        ...
                        if($divisi != '' OR in_array($position_id, array('65','66','67','68','71','103'))) { //sales jkt bikin cash
                            $this->notification($cash_id, $rec_id, $notif, $kadiv);
                        }elseif(in_array($position_id, array('88','89','90','91','93','100'))) { //kadiv bikin cash
                            $this->notification($cash_id, $rec_id, $notif, '2');
