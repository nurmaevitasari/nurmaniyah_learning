<?php if(! defined('BASEPATH')) exit('No direct script access allowed');
	...
	*/
	class M_purchasing extends CI_Model
	{
		...
		public function loadItemEdit($id)
		{
			$sql = "SELECT ve.*, lg.nickname FROM tbl_pr_vendor ve
					LEFT JOIN tbl_loginuser lg ON lg.karyawan_id = ve.user_approved
					WHERE ve.id = $id";
		...
		public function addPR()
		{
            ...
            if(in_array($position_id, array('1', '2'))) { //direksi bikin pr

   	
            }elseif(in_array($position_id, array('55', '56', '57', '58'))) { //kacab bikin pr
                    if($divisi == '') {
                        $this->newNotification($pr_id, $rec_id, $notif, '2');
                    }elseif($divisi != '') {
                        $this->newNotification($pr_id, $rec_id, $notif, $kadiv);
                    }
            }else {
                switch ($cabang) { //staff cabang bikin pr
                    case 'Bandung':
                        $this->newNotification($pr_id, $rec_id, $notif, '57');
                                      
                        break;
                    case 'Medan':
                        $this->newNotification($pr_id, $rec_id, $notif, '56');
                       
                        break;
                    case 'Surabaya':
                        $this->newNotification($pr_id, $rec_id, $notif, '55');
                       
                        break;        
                    case 'Cikupa':
                        $this->newNotification($pr_id, $rec_id, $notif, '58');
                      
                        break;
                    default:
                        
                        if(in_array($position_id, array('88','89','90','91','93','100'))) { //kadiv bikin pr
                            $this->newNotification($pr_id, $rec_id, $notif, '2');
                        }elseif(in_array($position_id, array('65','66','67','68','71','103'))) { //sales jkt bikin pr
                            $this->newNotification($pr_id, $rec_id, $notif, $kadiv);    
                        }elseif($divisi == '') { //staff jkt bikin pr (hrd, it, import, umum)
                           //echo "aaaaaa"; exit();
                            $this->newNotification($pr_id, $rec_id, $notif, '2');
                        }elseif (!empty($divisi)) {
                        	//echo "bbbbbbb"; exit();
            ...
            // ===============================================================================================================
             ...
             /*    if((!in_array($sess['position_id'], array('1','2','77','88','89','90','91','93','100')) AND !empty($divisi)) OR (in_array($sess['position_id'], array('65','66','67','68','71','72')) AND $sess['cabang'] == '')) // sales jakarta
                {
                ...
                elseif (in_array($sess['position_id'], array('5','7','8','9','11','12','76'))) { //staff jakarta. FA
		        	$this->notification($pr_id, '', '13', '2');
		        	//$this->notification($pr_id, '', '13', '3');        
                }
                else { //staff jakarta HRD IMPORT
                    $this->notification($pr_id, '', '13', '2');
                    //$this->notification($pr_id, '', '13', '3');
				} */
		...
		public function UpStatus($type, $id)
		{
			...
			if (in_array($position, array('1','2','77')))
			{
				...
				if($divisi AND empty($level) AND !empty($cbg) AND !in_array($pos_user, array('55','56', '57', '58', '95'))) { //staff cabang pilih divisi
					//echo "1111"; exit();
					$this->db->where('id', $id);
					$this->db->update('tbl_purchasing', array('level_approval' => 'Kacab'));
					
					$this->newNotification($id, $id, '13', $kadiv);

				}elseif($divisi AND empty($level) AND !empty($cbg)) { // kacab pilih divisi
					//echo "22222"; exit();
					$this->db->where('id', $id);
					$this->db->update('tbl_purchasing', array('level_approval' => 'Kadiv'));
					
					$this->newNotification($id, $id, '13', '2');
				...
				}elseif(empty($level) AND !empty($cbg)) { //staff cabang tidak pilih divisi
					//echo "4444"; exit();
					$this->db->where('id', $id);
					$this->db->update('tbl_purchasing', array('level_approval' => 'Kacab'));
				...
				}elseif(empty($level) AND empty($cbg) AND in_array($pos_user, array('65','66','67','68','71','103'))) { //sales jakarta
					//echo "5555"; exit();
					$this->db->where('id', $id);
					$this->db->update('tbl_purchasing', array('level_approval' => 'Kadiv'));
				...
				}elseif(empty($level) AND empty($cbg) AND $divisi) { //staff jakarta pilih divisi
                    //echo "666"; exit();
                    $this->db->where('id', $id);
					$this->db->update('tbl_purchasing', array('level_approval' => 'Kadiv'));
                    ...
                    //echo "666"; exit();
                    $this->db->where('id', $id);
					$this->db->update('tbl_purchasing', array('level_approval' => 'Dir'));	
		...
		public function uploadfile($type_id, $type, $sub_id)
		    ...
		    if($_FILES)
		    {
				...
				foreach ($_FILES['filepr']['name'] as $key => $value)
				{
					...
					if(!$value)
					{
						...
						$kar_id = $_SESSION['myuser']['karyawan_id'];
				        $logpr = $this->AddLogPR($type_id, $kar_id);
			    
			    		$addpsn = array(
							'pr_id' => $type_id,
							'log_pr_id'	=> $logpr,
							'sender' => $_SESSION['myuser']['karyawan_id'],
							'pesan'	=> 'Menambahkan File Pada Item ID <b style="color:green;">'.$sub_id.'</b>',
							'date_created'	=> date('Y-m-d H:i:s'),
							);
						$this->db->insert('tbl_pr_pesan', $addpsn);
						$pesan_id = $this->db->insert_id();
  		...
  		public function uploadfiles($type_id, $type, $sub_id)
  		{
		    function compress_image($src, $dest , $quality)
		    {
		        $info = getimagesize($src);
		...
		private function newNotification($pr_id, $rec_id, $notif, $position_cbg)
		{
			$date = date('Y-m-d H:i:s');
			$sql = "INSERT INTO tbl_notification (user_id, record_id, record_type, modul_id, status, modul, date_created) 
					SELECT id as user, '$rec_id', '$notif', '$pr_id', '0', '5', '$date' FROM tbl_karyawan
	    ...
	    {
	    	$sql="UPDATE `tbl_pr_vendor` SET `published`= '0' WHERE `id`='$id';";
	    	$query=$this->db->query($sql);
	    ...
	    {
	    	$kar_id = $_SESSION['myuser']['karyawan_id'];
	    	$sql="UPDATE `tbl_pr_vendor` SET `status_purchaser`='1' WHERE `id`='$id';";
	    	$query= $this->db->query($sql);
	    ...
	    public function Receive()
	    {
	    	...
	    	$kar_id = $_SESSION['myuser']['karyawan_id'];
	    		...
	    		$addpsn = array(
					...
					'log_pr_id'	=> $logpr,
					'sender' => $_SESSION['myuser']['karyawan_id'],
					'pesan'	=> 'Receive Item ID <b style="color:green;">'.$id.'</b>, Harga beli :<b style="color:green;">'.$harga_beli.'</b>',
					'date_created'	=> date('Y-m-d H:i:s'),
					);
				$this->db->insert('tbl_pr_pesan', $addpsn);
				$pesan_id = $this->db->insert_id();
	    ...
	    public function cancel($id)
  		{
		    ...
		    if($_SESSION['myuser']['karyawan_id'] == $res[0]['id_operator']) {
		      ...
		      $uplog = array(
		        'overto'      => '100',
		        'time_login'  => date('Y-m-d H:i:s'),
		        'timN['myuser']['karyawan_id'] != $res[0]['id_operator']) {
