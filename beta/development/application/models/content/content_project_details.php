...
<div id="page-inner">
	...
	<div class="col-sm-12">
		<dl style="font-size: 14px;" class="dl-horizontal">
			...
			<dt>Site CP</dt>
			<dd><?= !empty($detail['site_cp']) ? $detail['site_cp'] : '-'; ?> / <?= !empty($detail['no_hp']) ? $detail['no_hp'] : '-'; ?></dd>
			<dt>Site Location</dt>
			...
			<dt>Project Aging</dt>
			<dd><?php $datediff = datediff($detail['deadline_date'], date('Y-m-d H:i:s'));  
				if($detail['deadline_date'] > date('Y-m-d H:i:s'))
				{
					$plusmin = " +";
				}elseif($detail['deadline_date'] <= date('Y-m-d H:i:s')) {
					$plusmin = " -";
				}
				$datediff = $plusmin.$datediff['days_total']."d ".$datediff['hours']."h ".$datediff['minutes']."m";
				?>
				<?= !empty($detail['days_deadline']) ? $detail['days_deadline'] : '0'; ?>d / <?php echo $datediff;  ?>
			</dd>
			<dt>Progress</dt>
			<dd>   
				<div class="progress">
					...
					if($detail['last_progress'] >= '3' AND $detail['project_type'] == '1') {
					...
					if($detail['last_progress'] >= '5' AND $detail['project_type'] == '1') {
			...
			<dd><?php 
				...
				{
					$sts = date('d-m-Y', strtotime($detail['dates']));
						$now = date('d-m-Y');
						$diff = datediff($sts, $now); 
			
					if($now < $sts OR $sts == $now) {
						echo "<span style='color:blue;'>On-Schedule / +".$diff['days_total']."d </span>";
					}elseif($now > $sts) {
						echo "<span style='color:red;'>Overdue / -".$diff['days_total']."d </span>";
					}
				}else{
					echo "-";
				} ?></dd>
			<dt>Execution</dt>
			<dd><?php if($detail['execution'] == '0') { echo "Queue"; }elseif($detail['execution'] == '1'){ echo "Worked-On"; } ?>
			</dd>
			...
			<?php if(in_array($_SESSION['myuser']['position_id'], array('1', '2', '88', '14'))) { ?>
			<hr />
	...
	<?php if($_SESSION['myuser']['role_id'] != '15' AND ($detail['last_progress'] < 4 OR ($detail['last_progress'] >= 4 AND $countfiles == '2'))) { ?>
