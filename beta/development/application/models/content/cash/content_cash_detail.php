...
<div id="detail" class="row">
    ...
    <?php 
      ...
      $button = array (
          'approve1' => '<div style="float:right;">
            <a href="#" type="button" name="yes" class="btn btn-sm btn-success" title="Approve"  data-target="#myModalQty" data-toggle = "modal"><i class="glyphicon glyphicon-ok" ></i> Approve</a>&nbsp;&nbsp;
        ...
        <?php $no = 1;
         ?>
          <tr>
            ...
            <td class="bgcolor">
              <?php if($detail['approval_status'] == 0 AND $detail['amount']== 0 AND in_array($user['position_id'], array('1', '2', '77'))) { ?>
               <center><span ><?php echo number_format($detail['amount_approved']); ?></span></center>
              <?php }

              elseif($detail['approval_status'] == 0 AND in_array($detail['cabang'], array('Bandung', 'Medan', 'Cikupa', 'Surabaya', '')) AND in_array($user['position_id'], array('2','55', '56', '57', '58'))) { ?>
               <center><span ><?php echo number_format($detail['amount_approved']); ?></span></center>
            ...
            <td><?php if($detail['date_approved'] != '0000-00-00 00:00:00') {
              echo $detail['nickname']; ?> <br> <b>Tanggal :</b> <?php echo date('d/m/Y H:i:s', strtotime($detail['date_approved']));
              } ?> 
            </td>
  ...
  <div><h3> Cash Expenses </h3>
      <?php if(!empty($detail['status']) != '0' AND $detail['sales_id'] == $_SESSION['myuser']['karyawan_id']) { ?>
      ...
      <tbody>
        <?php $no = 1;
          ...
          <tr>
            ...
            <!-- <td><?php //echo $exp['amount_expense']; ?></td> -->
           ...
           <td><?php if(!empty($row['file_name'])) { ?>
                <a href="<?php echo $file_url.'assets/images/upload_cash/'.$row['file_name']; ?>" target="_blank"><?php echo $row['file_name'] ?></a><br>
            ...
            <td id="td_<?php echo $row['id'] ?>"><center>
                  <?php switch ($row['status_approved']) {
                    ...
                    default: ?>
                      <?php if(in_array($_SESSION['myuser']['position_id'], array('5', '18', '60', '62', '75','1', '2'))) { ?>
            ...
            <td style="text-align: right; font-weight: bold;" id="td2_<?php echo $row['id'] ?>">
              <?php if($row['status_approved'] == '1') {
                echo number_format($row['amount_expense']);
              }else {
                echo "0";
              } ?>   
            </td>
  ...
  <div class="total">
      ...
      if($minus == '-') {
        ...
        <label class="pull-right balance" style="font-size:17px; color:red;">Total :  Rp. <?php echo number_format($detail['total_amount']); ?>
      <?php }elseif($minus != '-' AND $detail['total_amount'] != '') { 
        $balance = $detail['total_amount'] - $detail['paid']; ?>
        <label class="pull-right balance" style="font-size:17px; color:blue;">Total :  Rp. <?php echo number_format($detail['total_amount']); ?>
      <?php }elseif($detail['type'] == '1' AND $detail['amount_approved'] != '0') { 
        $balance = $detail['amount_approved']; ?>
        <label class="pull-right balance" style="font-size:17px; color:blue;">Total :  Rp. <?php echo number_format($detail['amount_approved']);
      }else{ 
        $balance = "0"; ?>
        <label class="pull-right balance" style="font-size:17px; color:blue;">Total :  Rp. 0
      <?php } ?>
        </label>
      </div>
      <br>
      <div class="col-sm-12">
        <?php if($detail['paid'] == '0' AND $detail['status'] != '0' AND in_array($_SESSION['myuser']['position_id'], array('5', '18', '60', '62', '75')) AND !empty($exp)) { ?>
  ...
  <h3>Log & Message</h3>
  <span style="color: purple; font-size: 18px;">Contributor : </span>
  <span><?php if($loadContrib) {
        ...
        { ?>
          <tr>
            ...
            <td><b style="color: #3992b0;"><?php echo $psn['nickname']; ?></b><br>
              (<?php echo $psn['position']; ?>)
            </td>
            <td><?php echo $psn['pesan']; ?> 
            </td>
          </tr>
        <?php $no++; 
        } ?>
  ...
  if($detail['sales_id'] == $_SESSION['myuser']['karyawan_id']) { ?>
    <a href="<?php echo site_url('Cash/pr_finished/'.$detail['id']); ?>" class="btn btn-success" onclick="return confirm('Apakah Anda yakin Cash <?php echo $detail['tipe'] ?> ini telah selesai ?')">Cash <?php echo $detail['tipe'] ?> Finished</a>
