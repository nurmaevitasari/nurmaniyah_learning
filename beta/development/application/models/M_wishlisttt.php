	...
	class M_wishlist extends CI_Model
	{
		...
		public function getKaryawan($id)
        {
            $sql = "SELECT kr.id, nama, position FROM tbl_karyawan kr
                    ...
                    WHERE kr.published = '1' AND kr.position_id != '99' AND kr.id NOT IN (SELECT contributor FROM tbl_wish_contributor WHERE wish_id = $id GROUP BY contributor) ORDER BY nama ASC";
            $res = $this->db->query($sql)->result_array();
		...
		public function wishlist()
		{
			...
			$sql = "SELECT wi.*, lu.nickname as user, lw.nickname as wish_name, lp.nickname as name_appr, ws.date_created as dt_created, ls.nickname as name_status, kr.position_id FROM tbl_wishlist wi 
					...
					LEFT JOIN tbl_wish_contributor co ON co.wish_id = wi.id ";
			
			if($user['position_id'] == '1' OR $user['position_id'] == '2') {
				
					
			}elseif($pos == '13') {
				$sql .= " WHERE wi.user = '$kar' OR co.contributor = '$kar' OR position_id IN ('13', '102')";
			}elseif(in_array($pos, array('55', '56', '57', '58'))) {
				$sql .= " WHERE wi.user = '$kar' OR co.contributor = '$kar' OR kr.cabang = '$cbg'";
			}elseif ($pos == '88') {
				$sql .= " WHERE wi.user = '$kar' OR co.contributor = '$kar' OR position_id IN ('65','88')";
			}elseif ($pos == '89') {
				$sql .= " WHERE wi.user = '$kar' OR co.contributor = '$kar' OR position_id IN ('66','89')";
			}elseif ($pos == '90') {
				$sql .= " WHERE wi.user = '$kar' OR co.contributor = '$kar' OR position_id IN ('68','90')";
			}elseif ($pos == '91') {
				$sql .= " WHERE wi.user = '$kar' OR co.contributor = '$kar' OR position_id IN ('71','91','100','103')";
			}elseif ($pos == '93') {
				$sql .= " WHERE wi.user = '$kar' OR co.contributor = '$kar' OR position_id IN ('67','93')";
			}elseif ($pos == '100') {
				$sql .= " WHERE wi.user = '$kar' OR co.contributor = '$kar' OR position_id IN ('103','100')";	
			}else {
		...
		{
			$this->db->select('di.date_created, di.discuss, di.id, lg.nickname');
