...
class Crm_model extends CI_Model {
    ...
    var $column_search = array('cr.id','kr.nickname','cs.perusahaan', 'ncs.perusahaan', 'cs.pic', 'ncs.pic', 'cr.date_created', 'cr.divisi', 'cr.prospect', 'cr.competitor', 'cr.prospect_value', 'cr.last_followup', 'cr.progress', 'cr.date_closed', 'progress_sts', 'cr.status_show', 'cr.status_cancel', 'cr.sales_cabang');
    ...
    private function _get_datatables_query($cons = '')
    {
        ...
        $sql = "SELECT cr.id, cr.sales_id, cr.link_modul, cr.link_modul_id, cr.divisi, cr.prospect, cr.prospect_value, cr.deal_value, cr.special_note, cr.progress, cr.progress_sts, cr.competitor, cr.posibilities, cr.last_followup, cr.note, cr.status_closed, cr.date_created, cr.date_closed, cr.published, cr.status_show, cr.status_cancel, lgu.nickname as user_lastupdate, IF (cr.customer_type = 1, cs.perusahaan, ncs.perusahaan) AS perusahaan, IF (cr.customer_type = 1, cs.pic, ncs.pic) AS pic, kr.nickname, nt.notes, nt.date_created as date_notes, lg.nickname as user_notes, cr.sales_cabang
                        FROM tbl_crm cr 
    ...
    public function getDetail($id)
        ...
        public function checklink($id)
        {
            $sql = "SELECT IF(sps.id = 'NULL', '', sps.job_id) as job_id, li.link_to_id, li.link_to_modul as link_modul, sps.id FROM tbl_link li 
                LEFT JOIN tbl_crm crm ON (crm.id = li.link_from_id AND li.link_from_modul = '8') 
                LEFT JOIN tbl_sps sps ON (sps.id = li.link_to_id AND li.link_to_modul = '3') 
                WHERE li.link_from_id = $id AND sps.jenis_pekerjaan NOT IN (3,8)";
        ...
        public function getLog($id)
        {
            ...
            $sql = "SELECT lo.*, lg.nickname, ps.pesan, kr.cabang, up.file_name FROM tbl_crm_log lo
                    LEFT JOIN tbl_crm cr ON cr.log_crm_id = lo.id
                    LEFT JOIN tbl_crm_pesan ps ON ps.log_crm_id = lo.id
                    LEFT JOIN tbl_loginuser lg ON lo.user_id = lg.karyawan_id
                    LEFT JOIN tbl_crm_progress_approval ap ON (ap.id = lo.crm_type_id AND lo.crm_type = 'Approval Progress')
                    LEFT JOIN tbl_karyawan kr ON kr.id = ap.user
                    LEFT JOIN tbl_upload_crm up ON (up.id = lo.crm_type_id AND lo.crm_type = 'Upload')
                    WHERE lo.crm_id = $id GROUP BY lo.id ASC";
        ...
        public function selKaryawan($id)
        {
            $dtl = $this->getDetail($id);
            $sales_id = $dtl['sales_id'];

            $sql = "SELECT kr.id, nama, position FROM tbl_karyawan kr
                    LEFT JOIN tbl_position ps ON kr.position_id = ps.id
                    WHERE published = 1 AND kr.id NOT IN ('101', '109', '123', '133', '$sales_id') ORDER BY nama ASC";
            $res = $this->db->query($sql)->result_array();
        ...
        public function addProgress()
        {
            ...
            if($progress_num < $last_prgnum AND !in_array($_SESSION['myuser']['position_id'], array('88', '89', '90', '91', '93','100'))) {
                ...
                if($last_value != $value) {
                    $isi_pesan .= "Change Prospect Value from Rp. ".number_format($last_value, '0', ',', '.')." to Rp. ".number_format($value, "0", ",", ".")."<br>";
                }
                ...
                if(($cbg != '' AND $cbg != ' ') AND !in_array($_SESSION['myuser']['position_id'], array('55', '56', '57'))) { //echo "cabnag"; exit();
                    switch ($cbg) {
                        case 'Bandung':
                            $pos = '57';
                            break;
                ...
                $sql = "INSERT INTO tbl_notification (user_id, record_id, record_type, modul_id, status, modul, date_created) 
                    SELECT id as user, '$progress_approval_id', '13', '$crm_id', '0', '8', '$date' FROM tbl_karyawan
                        WHERE id NOT IN ('$user', '101', '123', '133', '109') AND position_id IN ('$pos') AND published = '1'
                        GROUP BY user ";
        ...
        public function ApprovalProgress($type, $crm_id, $appr_id ='', $log_id = '')
        {
            ...
            if($type == '2') { //echo "222"; exit();
                ...
                $this->db->where('id', $crm_id);
                $this->db->update('tbl_crm', array('progress_sts' => '3', 'status_closed' => '', 'date_closed' => '0000-00-00 00:00:00', 'posibilities' => $posibilities));
            ...
            }elseif ($type == '1' AND $_SESSION['myuser']['cabang'] == '') { //approve by kadiv
                $app = $this->getApprovalData($appr_id);
                ...
                $sql = "SELECT divisi FROM tbl_crm WHERE id = $crm_id";
                ...
                switch ($divisi) {
                    ...
                    $sql = "INSERT INTO tbl_notification (user_id, record_id, record_type, modul_id, status, modul, date_created) 
                        SELECT id as user, '$appr_id', '13', '$crm_id', '0', '8', '$date' FROM tbl_karyawan
                            WHERE id NOT IN ('$user', '101', '123', '133', '109') AND position_id IN ('$pos') AND published = '1'
                            GROUP BY user ";
                    $this->db->query($sql);     
            }
        ...
        public function notification($crm_id, $rec_id, $notif, $user = '')
        {
            ...
            if($a == '' OR $a == ' ' OR ($notif == '13' AND in_array($_SESSION['myuser']['position_id'], array('55', '56', '57'))))
            {
                $position_cbg = '';
            }elseif($a == 'Bandung') {
              $position_cbg = '57';
