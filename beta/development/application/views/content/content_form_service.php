<div class="form-group">
		...
		<div class="form-group row serial">
			...
			<div class="col-sm-3 tglbeli">
				<input type="text" class="form-control" name="tgl_beli" placeholder="dd/mm/yyyy" id="tgl_beli" required="required">
			</div>

		</div>	

		<div class="form-group row">
			<label class="control-label col-sm-2" id="area">Area Service</label>
			<div class="col-sm-8">
				<input type="text" class="form-control" name="areaservis" id="areaservis" required>
			</div>
		</div>
		<div class="form-group row">
			<label class="control-label col-sm-2" id = "freq">Frekuensi Kunjungan</label>
			<div class="col-sm-2">
				<input type="text" class="form-control" name="frekuensi" required><p style="font-size : 12px;">*Untuk kasus yang sama</p>
			</div>

			<label class="control-label col-sm-3" id = "schedule">Pengajuan Schedule service</label>
			<div class="col-sm-3">
		...
		<!-- <div class="form-group">
			<label class="control-label col-sm-2">FREE Servis</label>
			<div class="col-md-1">
					<div class="radio">
						<input type="radio" name="free_servis" value="1" required="true"> Ya
					</div>
				</div>
				<div class="col-md-1">
					<div class="radio">
						<input type="radio" name="free_servis" value="0" required="true"> Tidak
					</div>
				</div>
		</div>	-->
		<div class="form-group row">
			<label class="control-label col-sm-2">Uraian Pekerjaan</label>
			<div class="col-sm-8">
				<textarea name="sps_notes" id="InputMessage" class="form-control" rows="4" required></textarea><p style="font-size: 11.5px;">*Uraian Pekerjaan wajib diisi lengkap dan jelas sehingga mudah dimegerti oleh tim pelaksana. <br /> 
				&nbsp; Uraian Pekerjaan yang tidak jelas bisa mengakibatkan pekerjaan terhambat karena salah pemahaman. <br />
				&nbsp; Gunakan bahasa Indonesia yang baik & benar agar mudah dimengerti semua orang.<br />
				&nbsp; Dilarang menggunakan singkatan dan istilah-istilah bahasa pergaulan.</p>
			</div>
		</div>
		...
		<div class="form-group row overto">
	     	<label class="control-label col-sm-2">Over To :</label>
	      	<div class="col-sm-8 free free_0">
	        	<select class="form-control free_0 free" name="overto" id="overto" data-placeholder="<?php echo $_SESSION['myuser']['nama']?>" required="true">
	                <option value="">-Pilih-</option>
	                  <?php 
	                    ...
	                    {
	                    ?>
	                    <option id="overto_<?= $row['id']; ?>" <?php if(!empty($c_new_sps['id_operator'])){ if($c_new_sps['id_operator'] == $row['id']){ ?> selected <?php }} ?> value="<?= $row['id']; ?>">
	                    <?php echo $row['nama']; ?>
	                    </option>
	                    <?php 
	                    } 
      		...
      		<div class="col-sm-8 free free_1">
      			<input type="text" name="free_overto" readonly="true" id="free_overto" class="form-control free_1 free" value="NENENG NURYATI">
	            <input type="hidden" name="hdn_overto" id="hdn_overto" class="form-control free_1 free" value="9">
      		</div>
      	</div>	
	...
	<div class="form-group row overto">
      ...
      <div class="col-sm-8">
        <textarea type="text" id="msg" class="form-control" name="message" required></textarea>
      </div>
    </div>
...
<script type="text/javascript">
	...
	$('input[name="free_servis"]').click(function() {
		var free = $(this).attr("value");

		var inputValue = $(this).attr("value");
        var targetBox = $(".free_" + inputValue);
        $(".free").not(targetBox).hide();
        $(targetBox).show();
        if(free == '1') {
        	$("#overtotype").val('Staff Fin (AR)');
        }else if (free == '0') {
        	$("#overtotype").val(' ');
        }
	});
