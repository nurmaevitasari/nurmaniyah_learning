        ...
        <div class="panel-heading">   
          <div style="overflow: hidden;"> 
            ...
            <div style="float:right;">
              <?php if(in_array($_SESSION['myuser']['position_id'], array('1', '2', '77', '83', '14'))) { ?>
          ...
          <div class="panel-body panel-notif">
            <?php if($notif)
            {
              ...
              { 
                ...
                if($ntf['record_type'] == 1)
                {
                  ...
                  if($ntf['modul'] == '8') {
                    ...
                    $sql = "SELECT a.user_id, a.date_created, c.nickname, IF (b.customer_type = 1, cs.perusahaan, ncs.perusahaan) AS perusahaan FROM tbl_notification as a 
                      ...
                      LEFT JOIN tbl_customer cs ON (b.customer_id = cs.id AND b.customer_type = '1')
                      LEFT JOIN tbl_non_customer ncs ON (b.customer_id = ncs.id AND b.customer_type = '0')
                      LEFT JOIN tbl_crm_progress_approval cn ON cn.id = a.record_id
                ...
                }elseif($ntf['record_type'] == '29') {
                  $notif_id = $ntf['id'];

                  switch ($ntf['modul']) {
                    case '8':
                      $sql = "SELECT b.id, a.modul_id, a.user_id, a.date_created, IF (b.customer_type = 1, cs.perusahaan, ncs.perusahaan) AS perusahaan, IF (ap.status = 1, 'Diterima', 'Ditolak') AS status FROM tbl_notification as a 
                        LEFT JOIN tbl_crm as b ON b.id = a.modul_id
                        LEFT JOIN tbl_crm_progress_approval ap ON ap.id = a.record_id
                        LEFT JOIN tbl_customer cs ON (b.customer_id = cs.id AND b.customer_type = '1')
                        LEFT JOIN tbl_non_customer ncs ON (b.customer_id = ncs.id AND b.customer_type = '0')
                        WHERE a.id = $notif_id GROUP BY a.id DESC";
                      $apr_crm = $this->db->query($sql)->row_array();  
                      //penurunan progress CRM ID sekian >> PT.abc diterima.
                    break;
                  }
                }
                ...
                if($ntf['record_type'] == 1){ 
                 ...
                 <?php  } 
                  ...
                  switch ($ntf['modul']) {
                    case '8': ?>
                      <div class="alert alert-default" style="font-size: 11px; background-color: #cbd8f4;" >
                        <?php echo date('d-m-Y h:i:s', strtotime($apr_crm['date_created'])) ?> Penurunan Progress <b>CRM ID <?php echo $apr_crm['id'] ?></b> >> <?php echo $apr_crm['perusahaan'] ?> <b><?php echo $apr_crm['status'] ?></b>
                        <a href= "<?php echo site_url('home/go_crm/'.$ntf['modul_id'].'/29');?>" name="go" class="btn btn-default btn-xs">GO</a>
                      </div>
                    <?php break;
                  }
                }
    ...
    <div class="panel panel-default">
        ...
        <div class="panel-body panel-notes">
          <form action="<?php echo site_url('Home/saveNotes'); ?>" method="POST">
            <textarea class="personalnotes" rows="5" name ="txt" id="personalnotes"><?= !empty($notes['notes']) ? $notes['notes'] : ''; ?></textarea>
          </form>
        </div>
    </div>
      </div>
