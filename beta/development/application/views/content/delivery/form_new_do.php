...
$project_sess = $this->session->userdata('sess_project_id');
$replace_sess = $this->session->userdata('sess_do_replacement');

	if($project_sess['modul'] == '9')
	{
		$sess = $this->session->userdata('sess_project_id');
	}elseif($crm_sess['modul'] == '8') {	
		$sess = $this->session->userdata('sess_crm_id');
	}else {
		$sess['id'] = '';
		$sess['divisi'] = '';
		$sess['deal_value'] = '0';
...
<div id="page-inner">
    ...
    <hr />
	<form class="form-horizontal" method="post" role="form" action="<?php echo site_url('C_delivery/add/delv');  ?>" enctype="multipart/form-data" onsubmit="this.btn_save.disabled = true; this.btn_save.val = 'Saving...'; ">
			<h4><?php echo $this->session->flashdata('message'); ?></h4>
			<label>** Diisi Sesuai dengan Accurate **</label>
		<br>
		<br>
		<input type="hidden" name="jenis" value="<?php echo $this->uri->segment(2); ?>">
		<input type="hidden" name="form_type" value="<?php echo $this->uri->segment(3); ?>">
		<input type="hidden" name="sess_id" value="<?= !empty($sess['id']) ? $sess['id'] : ''; ?>">
		<input type="hidden" name="modul" value="<?= !empty($sess['modul']) ? $sess['modul'] : ''; ?>">
		...
		<div class=" form-group row">
			<label class="control-label col-sm-2">Delivery ID Replacement</label>
			<div class="col-sm-8">
					<?php if($replace_sess) { ?>
						<input type="text" class="form-control"  name="replacement_id" readonly="readonly" value="<?php echo $replace_sess; ?>">
		
					<?php }else {  ?> 
						<select class="form-control" style="width: 100%;" name="replacement_id">
							<option value="">-Pilih-</option>
							<?php foreach ($fin_do as $key => $val) { ?>
								<option value="<?php echo $val['id'] ?>">ID <?php echo $val['id'] ?> - <?php echo $val['no_so'] ?> (<?php echo $val['perusahaan'] ?>)</option>	
							<?php } ?>
						</select>
						<p>*Diisi bila melakukan Replacement Delivery.*</p>
					<?php } ?>
		...
		<div class="form-group">
			<label class="control-label col-sm-2">Divisi</label>
				<div class="col-md-1">
					<div class="radio">
						<input type="radio" name="divisi" value="dhc" required="" <?= ($sess['divisi'] == 'dhc') ? 'checked' : ''; ?>> DHC
					</div>
				</div>
				<div class="col-md-1">
					<div class="radio">
						<input type="radio" name="divisi" value="dre" required="" <?= ($sess['divisi'] == 'dre') ? 'checked' : ''; ?>> DRE
					</div>
				</div>
				<div class="col-md-1">
					<div class="radio">	
						<input type="radio" name="divisi" value="dce" required="" <?= ($sess['divisi'] == 'dce') ? 'checked' : ''; ?>> DCE
					</div>
				</div>
				<div class="col-md-1">
					<div class="radio">
						<input type="radio" name="divisi" value="dhe" required="" <?= ($sess['divisi'] == 'dhe') ? 'checked' : ''; ?>> DHE
					</div>
				</div>
				<div class="col-md-1">	 
					<div class="radio">
						<input type="radio" name="divisi" value="dgc" required="" <?= ($sess['divisi'] == 'dgc') ? 'checked' : ''; ?>> DGC
					</div>
				</div>
				<div class="col-md-1">
					<div class="radio">
						<input type="radio" name="divisi" value="dee" required="" <?= ($sess['divisi'] == 'dee') ? 'checked' : ''; ?>> DEE
					</div>
				</div>
				<div class="col-md-1">
					<div class="radio">
						<input type="radio" name="divisi" value="dwt" required="" <?= ($sess['divisi'] == 'dwt') ? 'checked' : ''; ?>> DWT
					</div>
				</div>
		</div>
		
		<div class="form-group">
			<label class="control-label col-sm-2">SO / Memo</label>
			<div class="col-md-1">	 
					<div class="radio">
						<input type="radio" name="rad" value="so" class="rad" required> SO
					</div>
				</div>
				<div class="col-md-1">
					<div class="radio">
						<input type="radio" name="rad" value="memo" class="rad" required> Memo
					</div>
				</div>
			...
			<label class="control-label col-sm-2">SO</label>
				<div class="col-sm-1">
					<input type="text" class="form-control inp cl-so" name="no_so1" id="no_so1">
				</div>
				<p style="font-size: 20px; width: 10px;" class="col-sm-1">/</p>
				<div class="col-sm-1">
					<input type="text" class="form-control inp cl-so" name="no_so2" id="no_so2">
				</div>
				<p style="font-size: 20px; width: 10px;" class="col-sm-1">/</p>
				<div class="col-sm-1" style="width: 110px;">
					<input  type="text" class="form-control inp cl-so" name="no_so3" id="no_so3">
				</div>
			</span>
			...
			<label class="control-label col-sm-2">Memo</label>
				<div class="col-sm-4">
					<input type="text" name="no_memo" class="form-control inp cl-memo" id="no_memo">
				</div>
			</span>
		</div>	
		...
		<div class="form-group" >
			...
			<div class="col-sm-8">
				<select name="pengiriman" class="form-control" id="pengiriman">
					<option value="">-Pilih-</option>							
					<option value="customer jemput">Customer Jemput</option>								
				...
				<input id="eksp" type="hidden" class="form-control" name="ekspedisi" placeholder="Ketik Nama Ekspedisi...">
			</div>						
		</div>
		...
		<div class="form-group row">
			...
			<div class="col-sm-3">
				<input type="text" class="form-control" name="gmap" id="gmap" placeholder="Gunakan URL Short / Singkat" required="true">
					<p style="font-size: 10px;">Wajib memasukkan link Google Map untuk mempermudah armada delivery </p>
			</div>
		</div>		
		<div class="form-group">
		...
		<div class="form-group">
			...
			<div class="col-sm-3">
				<input type="text" class="form-control" name="transaksi" id="transaksi" required="true" onkeyup="splitInDots(this)" value="<?= !empty($sess['deal_value']) ? number_format($sess['deal_value'], 0, ',', '.') : '' ?>">
			</div>
		</div>
...
$(document).ready(function() {
    ...
    $('input[name="rad"]').click(function(){
        ...
        $(targetBox).show();
        $(".cl-" + inputValue).attr('required', true);
        $(".inp").not(".cl-" + inputValue).attr('required', false);
