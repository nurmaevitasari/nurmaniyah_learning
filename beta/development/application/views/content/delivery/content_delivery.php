...
<div id="page-inner">
	...
	<ul id="myTab" class="nav nav-tabs">
        <li id="li_pending" class="active tabs"><a href="#home" data-target="#home" data-toggle="tab"><center><b>Pending</b></center></a></li>
        <li id="li_fix" class=" tabs"><a href="#profile" data-target="#profile" data-toggle="tab"><center><b>Fix</b></center></a></li>
    </ul>
		    	...
		    	<table class="table table-hover" id="kodok">
		      		<thead>
				        <tr>
					        ...
					        <th>Customer</th>
					        <th>Nama Item</th>
					        <th style="width: 45px;">Umur DO</th>
					        <th>Tanggal Kirim</th>
					        <th>Status</th>
					 	    <th>Action</th>
				        </tr>     
				    ...
				    <tbody>
				    <?php 
				    	$x = count($do); //print_r($x);exit();
				    	foreach ($do as $index => $row) {
				    ?>
				    		
				    		<tr>
					    		<td><?php $id = $row['id'];  echo $id; ?>
					    		<input type="hidden" name ="count_do" id = "count_do" class="count_do" value="<?php echo $x; ?>">
					    		<input type="hidden" id = "time_<?php echo $x; ?>" class="date_start_time" value="<?php echo $row['date_open']; ?>">	
					    		</td>
					    		...
					    		<td><?php $load_prd = $do[$index]['type_id']; 
						    		foreach ($load_prd as $prd) {
						    			echo "<li>".$prd['product']."<br>";
						    		} ?>
					    		</td>

					    		<td id = "elapsed_<?php echo $x; ?>" class="time-elapsed">
					    		...
					    		<?php echo date('d-m-Y', strtotime($row['tgl_estimasi'])); ?><br>
					    		...
					    		<?php if(in_array($pos, array('1', '2', '19', '55', '56', '57', '58', '59', '60', '62', '75', '77'))) { ?>
					    			ondblclick="changestatus('<?php echo $id; ?>')"
					    		<?php } ?>	
					    		>
					    			<b><?php echo $row['user_pending']; ?></b><br />
					    			<?php $csts = strtolower($row['status']); ?>
					    			<span id="sts_<?php echo $id; ?>" class="label <?php echo $csts; ?>"><?php echo $row['status']; ?></span><br>
					    			<?php if ($row['status'] != 'Waiting') { ?>
					    				<span style="font-size: 10px;">Last Updated: 
					    					<?php echo date('d-m-Y H:i:s', strtotime($row['date_created'])); ?>
					    					<br>
					    					<b>By : <?php echo $row['nickname']; ?></b>
					    				</span>
					    			<?php } ?>
					    		</td>
					    		<td>
					    			<a href="<?php echo site_url('C_delivery/details/'.$id) ?>" type="button" class="btn btn-default btn-sm btn-detail" title="Detail">Detail</a>
					    		</td>
				    	</tr>
 ...
 <!-- ############################## DO YANG TELAH TERJADWAL ################################## -->

    	<div class="tab-pane fade" id="profile">
   		<br><br>
		  	<div class="table-responsive" style="font-size: 12px;">
		    	<table class="table table-hover" id="kodok2">
		      		<thead>
				        <tr>
					        <th style = "width : 35px;">ID</th>
					        <th>No. SO</th>
				    ...
				    foreach ($do_jdwl as $index => $row) {  ?>
				    		<tr>
					    		...
					    		<input type="hidden" name="cat" id="cat_<?php echo $id; ?>" value="<?php echo $row['category']; ?>" >
						    		<?php echo date('d-m-Y', strtotime($row['tgl_estimasi'])); ?></span><br>
						    		<span style="font-size: 10px;">Last Updated: <br><?php echo date('d-m-Y H:i:s', strtotime($row['date_edit'])); ?><br> 
						    		<b>By : <?php echo $row['user_edit']; ?></b></span>
					    		</td>
					    		<td id= "chg_<?php echo $id; ?>" 
					    		...
					    		<td><a href="<?php echo site_url('C_delivery/details/'.$row['id']); ?>" type="button" class="btn btn-default btn-sm btn-detail" title="Detail">Detail</a>
					    		</td>
				    	</tr>
		...
		for (var ii = 1; ii <= co2; ii++) {
	    	$("#time_jdwl_" + ii).each(function(i) {
		        ...
		        var s = diff;
		       	...
		       	$('#elapsed_jdwl_'+ ii).html(d + "d " + h + "h " + m + "m ");
	   	 	});
	   	}
	}
} 
