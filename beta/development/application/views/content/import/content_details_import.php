...
<!-- /. ROW  -->
	...
	<dl class="dl-horizontal" style="font-size: 14px;">
		...
		<dt>Import ID Date</dt>
		<dd><?php echo date('d-m-Y H:i:s', strtotime($detail['date_created'])); ?></dd>
		<dt>PI Date</dt>
		<dd><?php echo date('d-m-Y H:i:s', strtotime($detail['date_pi'])); ?></dd>
		<dt>Shipment VIA</dt>
		<dd><?php echo $detail['ship_via']; ?></dd>
		<dt>Departure Date</dt>
		<dd><?php echo date('d-m-Y', strtotime($detail['dept'])); ?></dd>
		<dt>Arrival Date</dt>
		<dd><?php echo date('d-m-Y', strtotime($detail['arrival'])); ?></dd>
		<dt>Receiver</dt>
		<dd><?php echo $detail['nickname']; ?></dd>
		<dt>Arrival Destination</dt>
...
<br>
	...
	<div class="col-md-12">
		...
		<?php if($status['status'] == 1 AND $_SESSION['myuser']['position_id'] == '4'){ ?>
			<a href="javascript:;" data-toggle="modal" data-target="#myModal" class="btn btn-primary" id="addproduct"><span class="fa fa-plus"></span> Add Products</a>
		<?php } 
...
<!-- MODAL UNTUK ADD PRODUCT IMPORT -->
	<div class="modal fade" id="myModal" role="dialog" method="post">
	 ...
	 <div class="form-group row">
      ...
      <div class="col-sm-6">
        <textarea class="form-control productname" name="product_name" readonly="readonly" id="product_name" rows="3"></textarea>
      </div>
    </div>
...
<!-- TAMPILAN TABLE DETAIL PRODUCT IMPORT -->
		...
		<table class="col-sm-12 table" style="font-size: 12px;">
			...
			foreach($discuss as $row) { ?>
				<tr>
					...
					<td><?php echo $row['type'] == 1 ? 'Menambahkan file <a target="blank" href='.$file_url.'assets/images/upload_import/'.$row['discuss'].'>'.$row['discuss'].'</a>' : $row['discuss']; ?></td>
				</tr>
			<?php $no++; } ?>		
			</tbody>	
		</table>
		<button class="btn btn-info btn-xs" data-toggle="modal" data-target="#myModalMsg"> <b>+</b> Discussion</button>
		 <a href="javascript:;" data-toggle="modal" data-target="#myModalUpload" class="btn btn-xs btn-warning btn-files"><span class="fa fa-plus"></span> Files</a>
	</div>	
  ...
  <!-- MODAL UNTUK FORM BOOK -->
				...
				<form class="form-horizontal" method="post" action="<?php echo site_url('C_import/booking'); ?>">
					...
					<div class="modal-body">
						...
						<div class="col-md-7">	
							...
							<input type="hidden" name="imp_id" value="<?php echo $detail['id']; ?>" />
	...
	<div class="modal fade" id="myModalPesan" role="dialog" method="post">
      <div class="modal-dialog">
        <div class="modal-content">
          <form class="form-horizontal" method="post" action='<?php echo site_url('c_import/add_pesan'); ?>' onsubmit="this.submit_btn.disabled = true; this.submit_btn.val = 'Saving...'; ">
            <div class="modal-header">
              <h4>Message</h4>
            </div>
            ...
            <div class="modal-body">
              <div class="form-group">
                <div class="col-lg-12">
                  <textarea class="form-control" rows="5" name="msg" id="msg" required="true" ></textarea>
            ...
            <div class="modal-footer">
              <a class="btn btn-default pull-left" data-dismiss="modal">Close</a>
              <input type='submit' class='btn btn-info submit_btn' value='Save' name="submit_btn">
    ...
    <div class="modal fade" id="myModalUpload" role="dialog" method="post">
      <div class="modal-dialog">
        <div class="modal-content">
          <form class="form-horizontal" method="post" role="form" action="<?php echo site_url('C_upload/free_add_import');  ?>" enctype="multipart/form-data" onsubmit="this.submit.disabled = true; this.submit.val = 'Uploading...'; ">
            ...
            <div class="modal-body">   
              <div class="form-group row file-row" id="file-row-1" enctype="multipart/form-data">
                ...
                <input type="hidden" name ="id_import" value="<?php echo $detail['id']; ?>"> 
                <input type="hidden" name="tipe" value="1">
                <input type="hidden" name="import_pd_id" class="import_pd">      
              </div>
...
<?php $id_import = $detail['id'];?>
	...
	<!-- TAMPILAN MODAL UNTUK ADD MESSAGE DISCUSSION  -->
	<div class="modal fade" id="myModalMsg" role="dialog" method="post">
	    <div class="modal-dialog">
	      <div class="modal-content">
	        <form class="form-horizontal" method="post" action='<?php echo site_url('C_import/add_discussion'); ?>' onsubmit="this.submit_btn.disabled = true; this.submit_btn.val = 'Saving...'; ">
	          <div class="modal-header">
	          ...
	          <div class="modal-body">
	            <div class="form-group">
	              <div class="col-md-12">
	                ...
	                <textarea class="form-control textarea-control" rows="6" name="msg"  required="true" id="ds"></textarea>
...
$("#addproduct").click(function() {
	CKEDITOR.instances['product_name'].destroy();
});
