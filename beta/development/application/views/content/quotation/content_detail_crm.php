...
<div id="page-inner">
				...
				foreach ($log as $val) { ?>
					<tr>
						...
						<td><?php switch ($val['nickname']) {
							...
							default:
								echo ($val['crm_type'] == 'Upload') ? 'Menambahkan file <a href="'.$file_url.'assets/images/upload_crm/'.$val['file_name'].'"target="_blank" >'.$val['file_name'].'</a>' : $val['pesan'];
								break;
							} ?></td>
						<td><?php 
	...
	<?php if($_SESSION['myuser']['role_id'] != '15' AND ($con_published['published'] != 1 OR in_array($_SESSION['myuser']['position_id'], array('88', '89', '90', '91', '93', '100')))) { ?>
		...
		<?php if($detail['progress_sts'] != '1' AND empty($chklink)) { ?>
		...
		<?php }
         ...
         <?php if(in_array($_SESSION['myuser']['position_id'], array('1', '2', '14', '15','65','66','67','68','72','71','88','89','90','91','92','93','100','103'))) { ?>
         	<button class="btn btn-success btn-sm" data-toggle="modal" data-target="#modalSelectQuotation">Create Quotation</button>	
         <?php } ?>
		...
		<?php if($detail['progress'] == 'Deal' AND $detail['progress_sts'] != 1) { ?>
	...
	<br>
	<br>
    <div id="modalSelectQuotation" class="modal fade" role="dialog">
      <div class="modal-dialog">
        ...
        <div class="modal-content">
          <div class="modal-header">
            ...
            <h4 class="modal-title">Select Quotation Type</h4>
          </div>
          <div class="modal-body text-center">
            <p>
            	<a href="<?php echo site_url('quotation/project/'.$detail['id']); ?>" class="btn btn-success">Project Quotation</a>
            </p>
            <p>
            	<a href="<?php echo site_url('quotation/product/'.$detail['id']); ?>" class="btn btn-primary">Product Quotation</a>
            </p>
          </div>
...
<script type="text/javascript">
    ...
    $("#btnSave").click(function(e) {
    	var messageLength = CKEDITOR.instances['hasil'].getData().replace(/<[^>]>/gi, ' ').length;
    	var space = CKEDITOR.instances['hasil'].getData().replace(/&nbsp;/gi, '').length;
    ...
    $("#prgs_btn").click(function(e) {
    	var messageLength = CKEDITOR.instances['prgs_note'].getData().replace(/<[^>]>/gi, ' ').length;
    	var space = CKEDITOR.instances['prgs_note'].getData().replace(/&nbsp;/gi, '').length;
    ...
    $("#change_btn").click(function(e) {
    	var messageLength = CKEDITOR.instances['alasan-change'].getData().replace(/<[^>]>/gi, ' ').length;
    	var space = CKEDITOR.instances['alasan-change'].getData().replace(/&nbsp;/gi, '').length;
