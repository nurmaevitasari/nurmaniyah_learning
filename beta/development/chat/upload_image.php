...
if ((isset($_SESSION['chat']['karyawan_id'])) AND (isset($_SESSION['chat']['username'])) AND (isset($_SESSION['chat']['password'])))
{
	...
	if(isset($_FILES["imgFile"]["type"]))
	{
		$validextensions = array("jpeg", "jpg", "png");
		$temporary = explode(".", $_FILES["imgFile"]["name"]);
		$file_extension = end($temporary);
		
		//JIKA JENIS FILE SESUAI
		if ((($_FILES["imgFile"]["type"] == "image/png") || ($_FILES["imgFile"]["type"] == "image/jpg") || ($_FILES["imgFile"]["type"] == "image/jpeg") || ($_FILES["imgFile"]["type"] == "image/gif")) && in_array($file_extension, $validextensions)) 
		{
			...
			if ($_FILES["imgFile"]["error"] == 0)
			{
				...
				if ($count > 0)
				{
					...
					$ekstensi = strtolower(strrchr($_FILES['imgFile']['name'], '.'));
					$namafile = $karyawan_id.'-'.time().'-'.slug($_FILES['imgFile']['name']).''.$ekstensi;
					$targetPath = "uploads/".$namafile; // Target path where file is to be stored
					move_uploaded_file($sourcePath,$targetPath) ; // Moving Uploaded file

					processImg($namafile); //resizing and move to server
				
					$message = $namafile;
					$excerpt = '<img src="img/imgicon.jpg" /> Photo';
					...
					VALUES ('$chat_id', '$karyawan_id', 'image', '$message', 'unread')");
				}
				else
				{
