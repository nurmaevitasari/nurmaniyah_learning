...
class M_delivery extends CI_Model {
  ...
  public function _do($term)
  {
    ...
    if(in_array($_SESSION['myuser']['position_id'], array('88', '89', '93', '100'))) {
      ...
      $sql .= "LEFT JOIN tbl_multi_overto as ov ON (ov.type_id = do.id AND ov.type = '2')
      ...
      $sql .= "LEFT JOIN tbl_multi_overto as ov ON (ov.type_id = do.id AND ov.type = '2')
            LEFT JOIN tbl_karyawan as kar ON kar.id = do.sales_id
            LEFT JOIN tbl_karyawan as kov ON kov.id = ov.overto
            LEFT JOIN tbl_do_contributor as co ON (co.do_id = do.id AND co.published = 0)
            WHERE $where AND (kar.cabang = '$cbg' OR ov.overto = '$kar' OR kov.cabang = '$cbg' OR co.contributor = '$kar')
            GROUP BY do.id DESC ";

    }elseif($role_id == '11' OR in_array($pos_id, array('18', '19', '58', '82', '38'))) {
       
        $sql .= "LEFT JOIN tbl_multi_overto as ov ON (ov.type_id = do.id AND ov.type = '2')
            LEFT JOIN tbl_karyawan as kr ON kr.id = ov.overto
            LEFT JOIN tbl_do_contributor as co ON (co.do_id = do.id AND co.published = 0)
            WHERE $where AND (kr.cabang LIKE '%Cikupa%' OR kr.cabang LIKE '% %' OR kr.cabang like '%%' OR co.contributor = '$kar' )
            GROUP BY do.id DESC ";
      ...
      $sql .= "LEFT JOIN tbl_multi_overto as ov ON (ov.type_id = do.id AND ov.type = '2')
            LEFT JOIN tbl_do_contributor as co ON (co.do_id = do.id AND co.published = 0)
            WHERE $where AND (ov.overto = '$kar' OR do.sales_id = '$kar' OR co.contributor = '$kar')
            GROUP BY do.id DESC ";
    }
    $do = $this->db->query($sql)->result_array();
  ...
  public function _do_terjadwal()
  {
    ...
    }elseif(in_array($pos_id, array('18', '19','82')) OR $role_id == '11') {
        $sql = "SELECT do.id, do.no_so, do.date_open, do.date_close, do.customer_id, do.tgl_estimasi, do.date_edit, do.category, st.status, st.date_created, cs.perusahaan, lgn.nickname, lg.nickname as user_edit, lgp.nickname as user_pending, lgdo.nickname as sales, do.divisi
          FROM tbl_do_status as st
          LEFT JOIN tbl_do AS do ON st.do_id = do.id
          LEFT JOIN tbl_customer as cs ON cs.id = do.customer_id
          ...
          GROUP BY do.id DESC";
    }elseif(in_array($pos_id, array('1', '2', '77', '14', '6','58'))) {
      $sql = "SELECT do.id, do.no_so, do.date_open, do.date_close, do.customer_id, do.tgl_estimasi, do.date_edit, do.category, st.status, st.date_created, cs.perusahaan, lgn.nickname, lg.nickname as user_edit, lgp.nickname as user_pending, lgdo.nickname as sales, do.divisi
        FROM tbl_do_status as st
        LEFT JOIN tbl_do AS do ON st.do_id = do.id
        LEFT JOIN tbl_customer as cs ON cs.id = do.customer_id
  ...
  {
    $sql = "SELECT psn.id, psn.type_id, psn.log_type_id, psn.pesan, psn.date_created, lg.nickname, psn.pesan_type FROM tbl_multi_pesan as psn
   ...
   public function checkReplacement($do_id)
  {
   ...
   public function karyawanCon($do_id)
  {
      $sql = "SELECT id, nama FROM tbl_karyawan 
              WHERE published = '1' AND id NOT IN ('101', '109', '133', '123', '0') AND id NOT IN (SELECT contributor FROM tbl_do_contributor WHERE do_id = '$do_id' AND published = 0 GROUP BY contributor) ORDER BY nama ASC";
      $res = $this->db->query($sql)->result_array();

      return $res;
  }

   public function getCon($do_id)
  {
      $sql = "SELECT nickname FROM tbl_do_contributor co 
              LEFT JOIN tbl_loginuser lg ON lg.karyawan_id = co.contributor
              WHERE do_id = '$do_id' AND co.published = 0 GROUP BY contributor ORDER BY nickname  ASC";
      $res = $this->db->query($sql)->result_array();
  ...
  public function uploadfile($type_id, $jenis, $type)
  {
    ...
    if($_FILES)
    {
      ...
      {
        $temp_file_location = $_FILES['userfile']['tmp_name'][$key];
      ...
      public function notification($id, $msg_id, $notif)
      {
        ...
        if(!empty($position_cbg)) {
          $sql = "INSERT INTO tbl_notification (user_id, record_id, record_type, modul_id, status, modul, date_created)
                  ...
                  UNION SELECT contributor, '$msg_id', '$notif', '$id', '0', '2', '$date' FROM tbl_do_contributor
                  WHERE do_id = '$id' AND contributor != '$user' AND published = '0' GROUP BY contributor
  ...
  private function CheckAppr($id)
  {
    $sql = "SELECT lvl_approval, approval FROM tbl_do WHERE id = '$id'";
  ...
  public function do_cancel()
  {
    ...
    if(!empty($this->CheckAppr($do_id))) {
      $this->db->where('id', $do_id);
      $this->db->update('tbl_do', array('lvl_approval' => '', 'approval' => '0',));
    }
  ...
  public function addContributor($contributor = '', $do_id = '')
  {
        
    if($contributor == '' AND $do_id == '') {
    ...
    foreach ($contributor as $con) {
        ...
        $this->db->insert('tbl_do_contributor', $args);
      ...
      $this->db->insert('tbl_multi_pesan', $addpsn);
      ...
      $this->db->insert('tbl_notification', $notif_con);
   } 
   return $do_id;
  }
}
